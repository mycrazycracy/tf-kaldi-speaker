...
from dataset.kaldi_io import open_or_fd, read_mat_ark, read_vec_int, write_vec_flt
from six.moves import range

parser = argparse.ArgumentParser()
parser.add_argument("-g", "--gpu", type=int, default=-1, help="The GPU id. GPU disabled if -1.")
parser.add_argument("-m", "--min-chunk-size", type=int, default=10, help="The minimum length of the segments. Any segment shorted than this value will be ignored.")
...
if __name__ == '__main__':
    ...
    for index, (key, feature) in enumerate(read_mat_ark(args.rspecifier)):
        ...
        if feature.shape[0] > args.chunk_size:
            ...
            embeddings = trainer.predict_speaker(args.node,
                                         np.array(feature_array[:-1], dtype=np.float32),
                                         np.array(ali_array[:-1], dtype=np.int32),
                                         feature_length[:-1])
            embedding_last = trainer.predict_speaker(args.node, feature_array[-1], ali_array[-1], [feature_length[-1]])
            ...
            if len(embeddings.shape) == 2:
                ...
                # The speaker embedding is [d]
                embedding = np.sum(embeddings * feature_length, axis=0) / np.sum(feature_length)
            else:
                sys.exit("It is phone-related task. Use extract_mt_phone.py instead.")
        else:
            tf.logging.info("[INFO] Key %s length %d." % (key, feature.shape[0]))
            embedding = trainer.predict_speaker(args.node, feature, ali_value, [feature.shape[0]])
