...
#        "$nnetdir" "$feat" "ark:| copy-vector ark:- ark,scp:${dir}/xvector.JOB.ark,${dir}/xvector.JOB.scp"
    ...
    $cmd JOB=1:$nj ${dir}/log/extract.JOB.log \
      nnet/wrap/extract_mt_wrapper.sh --gpuid -1 --env $env --min-chunk-size $min_chunk_size --chunk-size $chunk_size \
        --normalize $normalize --node $node \
        "$nnetdir" "$feat" ${sdata}/pdf.scp "ark:| copy-vector ark:- ark,scp:${dir}/xvector.JOB.ark,${dir}/xvector.JOB.scp"
  fi
fi
