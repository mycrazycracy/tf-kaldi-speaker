...
if [ $# -ne 3 ]; then
  echo "Usage: $0 [options] <transition-model> (<lang-dir>|<graph-dir>) <decode-dir>"
  echo " Options:"
...
for f in $lang/words.txt $transdir/final.trans_mdl $dir/lat.1.gz $dir/num_jobs; do
...
$cmd JOB=1:$num_jobs $dir/log/get_lattice_stats.JOB.log \
  ali-to-phones --write-lengths=true "$transdir/final.trans_mdl" "ark:gunzip -c $dir/ali_tmp.JOB.gz|" ark,t:- \| \
...
$cmd JOB=1:$num_jobs $dir/log/lattice_best_path.JOB.log \
  ali-to-phones --per-frame=true "$transdir/final.trans_mdl" "ark:gunzip -c $dir/ali_tmp.JOB.gz|" ark,t:- \| \
